...
- `DWMAP` 수치지도 검색, 한번클릭으로 반경 1km 지역별 도엽번호 자동 탐색 및 새 빈 도면 원좌표 결합
- `DWJB` 한번클릭으로 주소 출력
...
`DWMAP`과 `DWJB`를 사용하려면 브이월드에서 필요한 자료를 내려받아 `C:\브이월드` 아래에 압축 해제합니다. 지역에 따라 폴더 이름이 달라도 DawooLISP가 하위 폴더를 자동으로 검색합니다.
...
- `DWMAP`은 `C:\브이월드` 아래에서 `1MAP5000_DXF_`가 포함된 폴더와 하위 DXF를 자동 검색합니다.
...
- 일부 자료는 GRS80 중부좌표로 제공되므로 `DWJB` 실행 시 현재 도면 좌표계에 맞게 서부·중부·동부 원점을 선택합니다.
